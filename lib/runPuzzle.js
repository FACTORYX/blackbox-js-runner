  ...
  box.once('finished', function(){
    ...
    box.nodes.forEach(function(v, k) {
      ...
      if (v.type === 'VariableDeclaration') {
        if (v.declarations == null) {
          return;
        }
        v.declarations.forEach(function(s, k) {
          var ref;
          if (((ref = s.id) != null ? ref.name : void 0) == null) {
            return;
          }
          return box.functions[s.id.name] = v.declarations;
        });
      }
      if (v.type === 'ExpressionStatement') {
        if (((ref = v.expression) != null ? (ref1 = ref.callee) != null ? ref1.name : void 0 : void 0) == null) {
          return;
        }
        return box.calls[v.expression.callee.name] = v.expression;
      }
    });
